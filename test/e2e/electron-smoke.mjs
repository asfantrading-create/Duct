...
await step('04-student', async () => { await page.getByRole('button', { name: /طالب جديد|New student/ }).click(); await page.locator('.modal input[type=text]').first().fill('سارة محمد'); await page.locator('.modal .btn.primary').click(); await page.waitForSelector('.module-card', { timeout: 15000 }); await page.waitForTimeout(600); const skip = page.locator('.modal .btn', { hasText: /تخطّي|Skip/ }); if (await skip.count()) await skip.click(); const info = await page.evaluate(() => window.duct.assignments.list()); if (!Array.isArray(info)) throw new Error('assignments IPC'); });
