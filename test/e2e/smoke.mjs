...
const dismissTour = async () => { const skip = page.locator('.modal .btn', { hasText: /تخطّي|Skip/ }); if (await skip.count()) { await skip.click(); await page.waitForTimeout(200); } };
await step('02-new-student', async () => { await page.getByRole('button', { name: /طالب جديد|New student/ }).click(); await page.locator('.modal input[type=text]').first().fill('أحمد علي'); await page.locator('.modal input[type=text]').nth(1).fill('2021001'); await page.locator('.modal .btn.primary').click(); await page.waitForSelector('.module-card', { timeout: 10000 }); await page.waitForTimeout(600); await shot('02b-tour'); await dismissTour(); });
...
await step('12b-guide-and-zoom', async () => { await nav(/دليل الاستخدام|User Guide/, '.prose'); await page.locator('.lesson-item').nth(4).click(); await page.waitForTimeout(200); await page.locator('.topbar .btn', { hasText: /^A\+$/ }).click(); await page.waitForTimeout(200); await page.locator('.topbar .btn', { hasText: /^A−$/ }).click(); await page.locator('.topbar .btn', { hasText: /شرح الشاشة|This screen/ }).click(); await page.waitForSelector('.modal .prose'); await page.locator('.modal .btn.primary').click(); });
await step('13-english', async () => { await page.locator('.topbar .btn', { hasText: /^EN$/ }).click(); await page.waitForSelector('.module-card, .card, .prose'); await page.waitForTimeout(400); });
if (role === 'supervisor') {
  await step('14-supervisor', async () => { await page.locator('.topbar .btn[title*="Switch"], .topbar .btn[title*="تبديل"]').first().click(); await page.waitForSelector('.auth-card'); await page.getByRole('button', { name: /Supervisor sign-in|دخول المشرف/ }).click(); await page.locator('.modal input[type=password]').nth(0).fill('1234'); await page.locator('.modal input[type=password]').nth(1).fill('1234'); await page.locator('.modal .btn.primary').click(); await page.waitForSelector('.module-card', { timeout: 10000 }); await dismissTour(); await page.locator('.nav-item', { hasText: /Supervisor|المشرف/ }).click(); await page.waitForSelector('table.tbl', { timeout: 10000 }); await page.getByRole('button', { name: /Details|التفاصيل/ }).first().click(); await page.waitForSelector('.modal'); await page.locator('.modal .btn.primary').click(); });
  await step('15-assign-exam', async () => { await page.locator('.tab', { hasText: /Assigned exams|الاختبارات المكلَّفة/ }).click(); await page.getByRole('button', { name: /New assigned exam|اختبار مكلَّف جديد/ }).click(); await page.locator('.modal input[type=text]').first().fill('Midterm HVAC'); await page.locator('.modal input[type=number]').first().fill('5'); await page.locator('.modal .btn.primary').click(); await page.waitForSelector('.card h3', { timeout: 10000 }); });
  await step('16-student-takes-assigned', async () => { await page.locator('.topbar .btn[title*="Switch"], .topbar .btn[title*="تبديل"]').first().click(); await page.waitForSelector('.profile-tile'); await page.locator('.profile-tile').first().click(); await page.waitForSelector('.module-card'); await dismissTour(); await page.waitForSelector('.alert.warn', { timeout: 5000 }); await page.locator('.nav-item', { hasText: /Assessments|الاختبارات/ }).click(); await page.waitForSelector('table.tbl'); await page.getByRole('button', { name: /^(Start|ابدأ)$/ }).first().click(); await page.waitForSelector('.q-card');
    for (let i = 0; i < 5; i++) { const opt = page.locator('.q-option').first(); if (await opt.count()) await opt.click(); else await page.locator('.q-card input[type=number]').fill('1'); const next = page.getByRole('button', { name: /^(التالي|Next)$/ }); if (await next.count()) await next.click(); else { await page.getByRole('button', { name: /إنهاء|Submit/ }).click(); const ok = page.locator('.modal .btn.primary'); if (await ok.count()) await ok.click(); } await page.waitForTimeout(120); }
    await page.waitForSelector('.score-ring', { timeout: 10000 }); await page.getByRole('button', { name: /Back to assessments|العودة إلى الاختبارات/ }).click(); await page.waitForSelector('table.tbl'); });
  await step('17-supervisor-sees-result', async () => { await page.locator('.topbar .btn[title*="Switch"], .topbar .btn[title*="تبديل"]').first().click(); await page.waitForSelector('.auth-card'); await page.getByRole('button', { name: /Supervisor sign-in|دخول المشرف/ }).click(); await page.locator('.modal input[type=password]').fill('1234'); await page.locator('.modal .btn.primary').click(); await page.waitForSelector('.module-card', { timeout: 10000 }); await dismissTour(); await page.locator('.nav-item', { hasText: /Supervisor|المشرف/ }).click(); await page.waitForSelector('.tab'); await page.locator('.tab', { hasText: /Assigned exams|الاختبارات المكلَّفة/ }).click(); await page.waitForSelector('.badge.ok, .badge.crit', { timeout: 10000 }); });
}
