...
const key = execFileSync(process.execPath, ['tools/issue-license.mjs', '--name', 'Shot University', '--type', 'lifetime', '--supervisor', '--out', path.join(OUT, 'shot.lic')], { cwd: root, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim().split('\n')[0];
...
{ const skip = page.locator('.modal .btn', { hasText: /تخطّي|Skip/ }); if (await skip.count()) { await skip.click(); await page.waitForTimeout(200); } }
...
await page.locator('.nav-item', { hasText: NAV[route] }).first().click(); await page.waitForTimeout(1500);
if (route === 'factory' || route === 'building') { const run = page.getByRole('button', { name: /^(تشغيل|Run)$/ }); if (await run.count()) await run.first().click(); }
for (const v of views) { if (v !== 'default') { const b = page.locator('.scene-tabs .btn', { hasText: new RegExp(v, 'i') }); if (await b.count()) await b.first().click(); } await page.waitForTimeout(2200); await page.screenshot({ path: path.join(OUT, `${route}-${String(views.indexOf(v) + 1).padStart(2, "0")}.png`) }); console.log("shot", views.indexOf(v) + 1, v); }
