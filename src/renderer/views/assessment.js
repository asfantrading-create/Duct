...
export default {
  ...
  async render(container, ctx, params = {}) {
    ...
    const showSetup = async () => {
      ...
      if (isSup) root.appendChild(h('div', { class: 'alert info', style: { marginBottom: '14px', alignItems: 'center' } }, icon('shield', 18), h('div', { style: { flex: 1 } },
        h('strong', null, tr('أنت مسجَّل كمشرف: الاختبارات المكلَّفة تُنشأ من لوحة المشرف، لا من هذه الصفحة.', 'You are signed in as supervisor: assigned exams are created from the Supervisor Dashboard, not from this page.')),
        h('div', { class: 'small', style: { marginTop: '4px' } }, tr('١ لوحة المشرف ← تبويب «الاختبارات المكلَّفة» ← «اختبار مكلَّف جديد»  ٢ حدّد العنوان والوحدات وعدد الأسئلة والوقت ودرجة النجاح  ٣ «إنشاء ونشر»؛ يظهر الاختبار للطلاب هنا مع زر «ابدأ» وتتابع نتائجهم من اللوحة.', '1 Supervisor Dashboard → “Assigned exams” tab → “New assigned exam”  2 Set title, modules, questions, time and pass mark  3 “Create & publish”; students see it here with a Start button and you track their results in the dashboard.'))),
        h('button', { class: 'btn primary', onClick: () => ctx.navigate('supervisor', { tab: 'assignments' }) }, icon('plus', 16), tr('إنشاء اختبار مكلَّف الآن', 'Create an assigned exam now'))));
      const assignedBox = h('div', { class: 'card', style: { marginBottom: '14px', borderColor: 'var(--brand-1)' } }, h('h3', null, icon('clipboard', 16), ' ', isSup ? tr('الاختبارات المكلَّفة المنشورة للطلاب', 'Assigned exams published to students') : tr('الاختبارات المكلَّفة من المشرف', 'Exams assigned by your supervisor')));
      if (!assignments.length) assignedBox.appendChild(h('p', { class: 'muted small' }, isSup ? tr('لا توجد اختبارات مكلَّفة منشورة بعد. أنشئ أول اختبار من لوحة المشرف بالزر أعلاه.', 'No assigned exams published yet. Create the first one from the Supervisor Dashboard with the button above.') : tr('لا توجد اختبارات مكلَّفة حالياً. عندما ينشئ المشرف اختباراً ستجده هنا مع زر «ابدأ».', 'No assigned exams right now. When your supervisor creates one it appears here with a Start button.')));
