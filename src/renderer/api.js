...
function createMock() {
  ...
  return {
    ...
    exporter: { csv: async ({ filename, headers, rows }) => { const esc = (v) => (v == null ? '' : /[",\n;]/.test(String(v)) ? `"${String(v).replace(/"/g, '""')}"` : String(v)); const body = [headers.map(esc).join(',')].concat(rows.map((r) => headers.map((hd, i) => esc(Array.isArray(r) ? r[i] : r[hd])).join(','))).join('\r\n'); return download(filename + (filename.endsWith('.csv') ? '' : '.csv'), '﻿' + body, 'text/csv'); }, json: async ({ filename, data }) => download(filename + (filename.endsWith('.json') ? '' : '.json'), JSON.stringify(data, null, 2), 'application/json'), png: async ({ filename, dataUrl }) => { const a = document.createElement('a'); a.href = dataUrl; a.download = filename; a.click(); return { ok: true, file: filename }; }, pdf: async ({ filename, html }) => { const w = window.open('', '_blank'); if (!w) return { ok: false }; w.document.write(html); w.document.close(); setTimeout(() => w.print(), 400); return { ok: true, file: filename }; }, defaultDir: async () => '(downloads)' },
