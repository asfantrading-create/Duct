 ...
 * Examples:
 *   node tools/issue-license.mjs --name "جامعة العلوم التطبيقية" --type lifetime --supervisor --seats 40
 *   node tools/issue-license.mjs --name "Duct Factory LLC" --type subscription --expires 2027-12-31 --modules DESIGN_LAB,FACTORY_TWIN
...
if (!args.name) { console.error('Usage: node tools/issue-license.mjs --name "Customer" [--type lifetime|subscription --expires YYYY-MM-DD] [--machine ID] [--modules A,B] [--supervisor] [--seats N] [--email x] [--notes x] [--key private.jwk.json] [--out file.lic]'); process.exit(1); }
...
const payload = buildPayload({ name: args.name, email: args.email, type: args.type || 'lifetime', expires: args.expires || null, machine: args.machine || null, notes: args.notes, modules, role: args.supervisor ? 'supervisor' : undefined, seats: args.seats });
